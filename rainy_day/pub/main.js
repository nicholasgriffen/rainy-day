...
function loadReadMe(login, repo) {
  const localReadMe = load(`${repo.name}-readMe`)
  // return promise-wrapped local value to support .then chaining
  if (localReadMe) {
    return Promise.resolve(localReadMe)
  } else {
    return github.client.getReadMe(login, repo.name)
      .then((readMe) => {
        save(`${repo.name}-readMe`, readMe.content)
        save(`${repo.name}-readMe-sha`, readMe.sha)
        save(`${repo.name}-readMe-path`, readMe.path)
